...
const ServiceModal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      <div className="absolute inset-0 backdrop-blur-md bg-white/20" onClick={onClose}></div>
      <div
...
const NonLegalServices = () => {
  ...
  const handleBookService = (service, isExpressService = false) => {
    navigate(
      `/booking?serviceID=${encodeURIComponent(service.serviceID)}&express=${isExpressService ? "true" : "false"}`
    );
  };
