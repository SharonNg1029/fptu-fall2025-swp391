...
const ConfirmBookingModal = ({
  ...
  const translateServiceNameToPDF = (serviceName) => {
    const serviceTranslations = {
      ...
      "Non-Invasive Relationship Testing (NIPT)":
        "Xét nghiệm ADN Huyết thống Không xâm lấn",
  ...
  const formatCurrency = (amount) =>
    amount ? `${Number(amount).toLocaleString()} đ` : "0 đ";
  ...
  const getCostBreakdown = () => {
    ...
    if (currentMedicationMethod === "staff-collection") {
      mediationCost = currentExpressService ? 0 : 500000;
    } else if (currentMedicationMethod === "postal-delivery") {
      mediationCost = currentExpressService ? 0 : 250000;
    } else if (currentMedicationMethod === "express") {
      mediationCost = currentExpressService ? 0 : 700000;
    }
  ...
  const handleSignatureComplete = async () => {
    ...
    try {
      ...
      if (!validateSignatureQuality(signatureData)) {
        message.error("Signature is too simple or unclear. Please sign again!");
        return;
      }
  ...
  const handleVNPAYPayment = async () => {
    ...
    try {
      ...
      processingMsg = message.loading(
        "Processing VNPAY payment, please wait a moment...",
        0
      );
      ...
      if (bookingData.firstPerson?.dateOfBirth) {
        if (
          typeof bookingData.firstPerson.dateOfBirth === "object" &&
          bookingData.firstPerson.dateOfBirth.format
        ) {
          firstPersonBirthDate =
            bookingData.firstPerson.dateOfBirth.format("YYYY-MM-DD");
        } else {
      ...
      if (bookingData.secondPerson?.dateOfBirth) {
        if (
          typeof bookingData.secondPerson.dateOfBirth === "object" &&
          bookingData.secondPerson.dateOfBirth.format
        ) {
          secondPersonBirthDate =
            bookingData.secondPerson.dateOfBirth.format("YYYY-MM-DD");
        } else {
      ...
      console.log("Sending VNPAY payment request with payload:", payload);
  ...
  const generatePDF = async (shouldDownload = true) => {
    ...
    try {
      ...
      if (finalBookingData?.signature) {
        ...
        console.log("✓ Signature retrieved from finalBookingData");
      ...
      const docDefinition = {
        content: [
            ...
            columns: [
              {
                ...
                text: [
                  "Tôi tên là (viết hoa): ",
                  {
                    text: (firstPerson?.fullName || "").toUpperCase(),
                    color: "#e91e63",
                    bold: true,
                  },
                  "    Giới tính: ",
          ...
          },
          {
            text: [
              "Số CCCD/CMND: ",
              {
                text: firstPerson?.personalId || "Chưa cung cấp",
                color: "#e91e63",
                bold: true,
              },
              "    Số điện thoại: ",
              {
                text: firstPerson?.phoneNumber || "",
                color: "#e91e63",
                bold: true,
              },
            ],
          ...
          },
          {
            text: [
              ...
              "    Tên dịch vụ: ",
              {
                text: translateServiceNameToPDF(
          ...
          },
          {
            text: [
              ...
              {
                text: translateCollectionMethodToPDF(
                  bookingData.collectionMethod?.name || ""
                ),
                color: "#2196f3",
                bold: true,
              },
              "    Kit xét nghiệm: ",
              {
                text: bookingData.selectedKitType
                  ? kitTypes.find(
                      (k) => k.value === bookingData.selectedKitType
                    )?.label || bookingData.selectedKitType
                  : "",
                color: "#2196f3",
                bold: true,
              },
            ...
            table: {
              ...
              body: [
                [
                  ...
                  { text: "Ngày thu thập", style: "tableHeader" },
                ],
                [
                  "1",
                  {
                  ...
                  {
                    text: formatDate(firstPerson?.dateOfBirth),
                    color: "#e91e63",
                    bold: true,
                  },
                  {
                ...
                [
                  "2",
                  {
                    text: (secondPerson?.fullName || "").toUpperCase(),
                    color: "#e91e63",
                    bold: true,
                  },
                  {
                    text: formatDate(secondPerson?.dateOfBirth),
                    color: "#e91e63",
                    bold: true,
                  },
                  {
          ...
          },
          {
            ol: [
              ...
              {
                text: "Tôi đã đọc và chấp nhận các điều khoản của Viện tại trang 02 và 03 của đơn này và tôi đồng ý để Genetix thực hiện các phân tích ADN với các mẫu trên. Nếu vi phạm, tôi xin chịu hoàn toàn trách nhiệm trước pháp luật.",
                margin: [0, 0, 0, 1],
              },
            ],
            margin: [20, 0, 20, 3],
          },
          {
            ...
            layout: {
              hLineWidth: function (i, node) {
                return i === node.table.body.length - 2 ||
        ...
        styles: {
          ...
          termsHeader: { fontSize: 14, bold: true, color: "#1976d2" },
          tableHeader: {
            fillColor: "#1976d2",
            color: "white",
            bold: true,
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <div
                ...
                <Text strong style={{ fontSize: 14 }}>
                  {selectedKitType
                    ? kitTypes.find((k) => k.value === selectedKitType)?.label
                    : "—"}
                </Text>
              </div>
            </Col>
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <div
                ...
                <Text strong style={{ fontSize: 14 }}>
                  {getMediationLabel(
                    medicationMethod || bookingData.selectedMedicationMethod
                  )}
                </Text>
              </div>
            </Col>
            <Col span={12}>
              <div
                style={{
                  padding: "12px 16px",
                  backgroundColor: isExpressService ? "#fff2e8" : "#fafafa",
                  borderRadius: 8,
                  border: `1px solid ${
                  ...
                  <Row gutter={8}>
                    <Col span={12}>
                      ...
                      <Text style={{ fontSize: 13 }}>
                        {firstPerson?.dateOfBirth?.format
                          ? firstPerson.dateOfBirth.format("DD/MM/YYYY")
                    ...
                    <Col span={12}>
                      ...
                      <Text style={{ fontSize: 13 }}>
                        {firstPerson?.gender
                          ? firstPerson.gender.charAt(0).toUpperCase() +
                            firstPerson.gender.slice(1)
                          : ""}
                      </Text>
                    </Col>
                  </Row>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  <Row gutter={8}>
                    <Col span={12}>
                      ...
                      <br />
                      <Text style={{ fontSize: 13 }}>
                        {secondPerson?.dateOfBirth?.format
                          ? secondPerson.dateOfBirth.format("DD/MM/YYYY")
                    ...
                    <Col span={12}>
                      ...
                      <Text style={{ fontSize: 13 }}>
                        {secondPerson?.gender
                          ? secondPerson.gender.charAt(0).toUpperCase() +
                            secondPerson.gender.slice(1)
                          : ""}
                      </Text>
                    </Col>
                  </Row>
            ...
            {paymentMethod === "cash" ? (
              <div
                ...
                <Text strong style={{ fontSize: 16, color: "#52c41a" }}>
                  💵 Cash payment on service receipt
                </Text>
              </div>
            ) : (
              <div
        ...
        return (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <Text type="secondary" style={{ fontSize: "12px" }}>
              💡 You can download the PDF file later in the booking history
              section.
            </Text>
          </div>
        );
...
const BookingPage = () => {
  ...
  const [selectedCollectionMethod, setSelectedCollectionMethod] =
    useState(null);
  const [selectedMedicationMethod, setSelectedMedicationMethod] =
    useState("walk-in");
  ...
  const updateFirstPersonPersonalIdVisibility = (dateOfBirth) => {
    ...
    setShowFirstPersonPersonalId(
      age !== null && age > 15 && relationship === "Child"
    );
  };
  ...
  const currentServicesData =
    selectedServiceType === "legal" ? legalServicesData : nonLegalServicesData;
  const currentCollectionMethods =
  ...
  const calculateTotalCost = () => {
    let serviceCost =
      selectedService && selectedService.basePrice
        ? Number(selectedService.basePrice)
        : 0;
    let collectionCost = 0;
    if (!isExpressService) {
      collectionCost =
        selectedCollectionMethod && selectedCollectionMethod.price
          ? Number(selectedCollectionMethod.price)
          : 0;
    }
  ...
  const getValidRelationshipsForService = useCallback((serviceName) => {
    ...
    if (serviceName === "DNA Testing for Inheritance or Asset Division") {
      ...
      if (validRelationships.has("Grandparent"))
        orderedRelationships.push("Grandparent");
      if (validRelationships.has("Grandchild"))
        orderedRelationships.push("Grandchild");
      if (validRelationships.has("Sibling"))
        orderedRelationships.push("Sibling");
  ...
  const getValidRelationshipsForSecondPerson = useCallback(
    (serviceName, firstPersonRelationship) => {
  ...
  const [
    availableSecondPersonRelationships,
    setAvailableSecondPersonRelationships,
  ] = useState([
    "Father",
    "Mother",
    "Child",
  ...
  const validateAge18 = (_, value) => {
    ...
    if (age < 18) {
      return Promise.reject(
        new Error("Representative must be 18 years or older!")
      );
    }
  ...
  const validateGenderRelationship = (_, value) => {
    if (!value) {
      return Promise.reject(new Error("Please select a relationship!"));
    }
  ...
  const validateSecondPersonGenderRelationship = (_, value) => {
    ...
    const firstPersonRelationship = form.getFieldValue([
      "firstPerson",
      "relationship",
    ]);
    if (firstPersonRelationship && value && selectedService?.name) {
      if (
        ...
        if (selectedService.name === "DNA Testing for Birth Certificate") {
          ...
          return Promise.reject(
            new Error(
              `With relationship"${firstPersonRelationship}" of the first person, the second person must choose one of the relationships: ${validOptions.join(
                ", "
              )}`
            )
          );
          ...
          const validPairs =
            serviceRelationshipMap.get(selectedService.name) || [];
          ...
          return Promise.reject(
            new Error(
              `Invalid relationship pair for this service. Valid pairs:${pairNames}`
            )
          );
        }
      }
  ...
  useEffect(() => {
    if (serviceID) {
      let foundService = legalServicesData.find(
        (service) => service.serviceID === serviceID
      );
      let serviceType = "legal";

      if (!foundService) {
        foundService = nonLegalServicesData.find(
          (service) => service.serviceID === serviceID
        );
  ...
  const handleExpressServiceChange = (checked) => {
    ...
    if (checked) {
      showNotification(
        "info",
        "Express Service activated! Please select your preferred Collection Method below. All collection methods will be FREE (0 VND)",
        6000
      );
      ...
      if (
        ...
        showNotification(
          "warning",
          "Please select a Medication Method for At Home collection with Express Service!",
          5000
        );
      }
    } else {
      ...
      showNotification(
        "info",
        "You have unchecked Express Service. Sample collection fee will be charged at original price.",
        5000
      );
    }
  };
  ...
  useEffect(() => {
    if (selectedService?.name) {
      const validRelationships = getValidRelationshipsForService(
        selectedService.name
      );
  ...
  useEffect(() => {
    try {
      ...
      if (selectedService?.name && firstPersonRelationship) {
        ...
        setAvailableSecondPersonRelationships(
          validSecondRelationships.length > 0
            ? validSecondRelationships
            : availableRelationships
        );

        const currentSecondRelationship = form.getFieldValue([
          "secondPerson",
          "relationship",
        ]);
        if (
          currentSecondRelationship &&
          !validSecondRelationships.includes(currentSecondRelationship)
        ) {
          form.setFieldsValue({
            secondPerson: {
              ...form.getFieldValue("secondPerson"),
    ...
    availableRelationships,
    form,
    getValidRelationshipsForSecondPerson,
  ]);
  ...
  useEffect(() => {
    if (selectedService?.name === "Non-Invasive Relationship Testing (NIPT)") {
      ...
      showNotification(
        "warning",
        ' NIPT service can only be performed at medical facilities. Collection method has been automatically set to "At Facility".',
        5000
      );
      showNotification(
        "info",
        "NIPT testing requires one Father and one Mother. For NIPT, only Mother can provide Amniotic Fluid sample, while Father must provide either Blood, Buccal Swab, Hair, or Nail sample.",
        7000
      );
    }
  ...
  useEffect(() => {
    if (selectedMedicationMethod === "postal-delivery") {
      ...
      showNotification(
        "info",
        "Postal Delivery requires payment via VNPAY. Payment method has been automatically set.",
        5000
      );
    }
  ...
  useEffect(() => {
    if (selectedService?.name === "Non-Invasive Relationship Testing (NIPT)") {
      ...
      const secondPersonRelationship = form.getFieldValue([
        "secondPerson",
        "relationship",
      ]);
      ...
      if (secondPersonRelationship === "Mother") {
        ...
        message.info(
          "For NIPT service, Mother must use Amniotic Fluid sample."
        );
      } else if (secondPersonRelationship === "Father") {
        const currentSampleType = form.getFieldValue([
          "secondPerson",
          "sampleType",
        ]);
        if (!currentSampleType || currentSampleType === "Amniotic Fluid") {
          ...
          message.info(
            "For NIPT service, Father cannot use Amniotic Fluid. Sample type has been set to Blood."
          );
        }
      }
    }
  ...
  const handleConfirmBooking = (values) => {
    ...
    if (selectedService?.name === "Non-Invasive Relationship Testing (NIPT)") {
      if (
        values.firstPerson?.relationship === "Father" &&
        values.firstPerson?.sampleType === "Amniotic Fluid"
      ) {
        form.setFieldsValue({
          firstPerson: {
            ...form.getFieldValue("firstPerson"),
        ...
        showNotification(
          "warning",
          "For NIPT service, Father cannot provide Amniotic Fluid sample. Sample type has been automatically changed to Blood."
        );
        return;
      }
      if (
        values.firstPerson?.relationship === "Mother" &&
        values.firstPerson?.sampleType !== "Amniotic Fluid"
      ) {
        form.setFieldsValue({
          firstPerson: {
        ...
        showNotification(
          "info",
          "For NIPT service, Mother must provide Amniotic Fluid sample. Sample type has been automatically corrected."
        );
        return;
      }
      if (
        values.secondPerson?.relationship === "Father" &&
        values.secondPerson?.sampleType === "Amniotic Fluid"
      ) {
        form.setFieldsValue({
          secondPerson: {
            ...form.getFieldValue("secondPerson"),
      ...
      if (
        values.secondPerson?.relationship === "Mother" &&
        values.secondPerson?.sampleType !== "Amniotic Fluid"
      ) {
        form.setFieldsValue({
          secondPerson: {
    ...
    if (!selectedMedicationMethod) {
      showNotification(
        "error",
        "Please select medication method! This field is required."
      );
      return;
    }
    if (!selectedKitType) {
    ...
    if (isExpressService && !selectedCollectionMethod) {
      showNotification(
        "error",
        "When using Express Service, you must choose a Collection Method!"
      );
      return;
    }
    ...
    if (
      ...
      showNotification(
        "error",
        "When using Express Service with At Home collection, you must select a Medication Method!"
      );
      return;
    }
    ...
    if (
      ...
      showNotification(
        "error",
        "Please enter your home address when choosing home pickup or home delivery!"
      );
      return;
    }

    if (
      !values.firstPerson?.fullName ||
      values.firstPerson.fullName.trim() === ""
    ) {
      showNotification(
        "error",
        "Please enter the first and last name of the person!"
      );
      return;
    }
    ...
    if (!values.firstPerson?.personalId) {
      showNotification(
        "error",
        "Please enter the first person is Personal ID number!"
      );
      return;
    }

    if (
      !values.secondPerson?.fullName ||
      values.secondPerson.fullName.trim() === ""
    ) {
      showNotification(
        "error",
        "Please enter the second person is first and last name!"
      );
      return;
    }
    if (!values.secondPerson?.dateOfBirth) {
      showNotification(
        "error",
        "Please select second person is date of birth!"
      );
      return;
    }
  ...
  function buildBookingPayload(data) {
    ...
    return {
      ...
      mediationMethod:
        data.medicationMethod || data.selectedMedicationMethod || "",
      additionalCost: 0,
  ...
  const getSuccessMessage = () => {
    ...
    const location =
      collectionMethod?.name === "At Home" ? "at home" : "at medical facility";
    const appointmentInfo =
      appointmentDate && timeSlot
        ? `on time ${moment(appointmentDate).format(
            "DD/MM/YYYY"
          )} at the time ${timeSlot}`
        : "on time";
  ...
  const handleBookingComplete = async (finalBookingData) => {
    try {
      ...
      let errorMessage =
        "An error occurred while saving the booking information!";
      if (error.response) {
      ...
      try {
        ...
        if (vnpResponseCode === "00") {
          ...
          if (!currentBooking) {
            currentBooking = pendingBookings.find((booking) => {
              const match =
                booking.customerID && orderInfo.includes(booking.customerID);
              console.log(
                `🔎 Checking customerID: ${booking.customerID} - Match: ${match}`
              );
              return match;
            });
          }
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="mb-4">
              ...
              <div className="flex space-x-4">
                <button
                  onClick={() =>
                    !isServicePreSelected && setSelectedServiceType("legal")
                  }
                ...
                <button
                  onClick={() =>
                    !isServicePreSelected && setSelectedServiceType("non-legal")
                  }
                  disabled={isServicePreSelected}
                  className={`px-4 py-2 rounded-md transition-all ${
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-600 mt-1">
                      This service only allows to choose one of two relationship
                      pairs: Father - Child or Mother - Child
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm mt-1 text-orange-600">
                        Fast and high priority processing - Results in the
                        shortest time
                      </p>
                    ...
                    {isStandardPreSelected && (
                      <div className="flex items-center text-orange-400"></div>
                    )}
                  </div>
                </label>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div
                  onClick={() => {
                    if (
                      ...
                      showNotification(
                        "warning",
                        'NIPT service can only be performed at medical facilities. Please select "At Facility" instead.',
                        4000
                      );
                      return;
                    }
                ...
                <div
                  onClick={() => {
                    ...
                    showNotification(
                      "success",
                      "Selected: At Facility collection method",
                      3000
                    );
                  ...
                  <p className="text-sm text-gray-600 mb-2">
                    Visit our facility for sample collection
                  </p>
                  ...
                  <div className="flex items-start mb-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-blue-600 mb-3">
                        Please provide your exact home address.
                      </p>
                    </div>
                  </div>
                  <textarea
                  ...
                  <div className="space-y-3">
                      ...
                      <div
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              Professional staff will collect samples
                            </p>
                            {isExpressService ? (
                    ...
                    {selectedCollectionMethod?.name === "At Home" &&
                      ...
                      !isExpressService && (
                        <div
                          onClick={() =>
                            setSelectedMedicationMethod("postal-delivery")
                          }
                          className={`p-4 border rounded-lg transition-all cursor-pointer ${
                            selectedMedicationMethod === "postal-delivery"
                              ? "border-blue-500 bg-blue-50"
                          ...
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600">
                                Sample collection kit delivered by post
                              </p>
                              {isExpressService ? (
                                <p className="text-sm font-semibold text-green-600">
                                  Free
              ...
              {selectedMedicationMethod && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-600 mb-3">
                    📋Select the type of kit that will be used for both test
                    participants.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {kitTypes.map((kit) => (
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-blue-700 mb-1">
                            Kit selected:{" "}
                            {
                              kitTypes.find((k) => k.value === selectedKitType)
                                ?.label
                            }
                          </p>
                    ...
                    <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="flex items-center">
                        ...
                        <span className="text-blue-800 font-medium">
                          With postal delivery, you just need to choose a date.
                          The kit will be delivered to your address on the
                          selected date.
                        </span>
                      </div>
                    </div>
            ...
            <Form
                    ...
                    <Form.Item
                      ...
                      <Select
                        ...
                        onChange={(value) => {
                          ...
                          const dateOfBirth = form.getFieldValue([
                            "firstPerson",
                            "dateOfBirth",
                          ]);
                          if (dateOfBirth) {
                          ...
                          if (selectedService?.name && value) {
                            const validSecondRelationships =
                              getValidRelationshipsForSecondPerson(
                                selectedService.name,
                                value
                              );
                            ...
                            const currentSecondRelationship =
                              form.getFieldValue([
                                "secondPerson",
                                "relationship",
                              ]);
                            if (
                              currentSecondRelationship &&
                              !validSecondRelationships.includes(
                                currentSecondRelationship
                              )
                            ) {
                              form.setFieldsValue({
                          ...
                          if (
                            ...
                            if (value === "Mother") {
                              ...
                              if (
                                !currentSampleType ||
                                currentSampleType === "Amniotic Fluid"
                              ) {
                                form.setFieldsValue({
                                  firstPerson: {
                                    ...form.getFieldValue("firstPerson"),
                      ...
                      rules={[
                        ...
                        ({ getFieldValue }) => ({
                          validator(_, value) {
                            ...
                            const relationship = getFieldValue([
                              "firstPerson",
                              "relationship",
                            ]);
                            ...
                            if (
                              ...
                              return Promise.reject(
                                "For NIPT service, Father cannot provide Amniotic Fluid sample"
                              );
                            }
                            return Promise.resolve();
                          },
              ...
              <Card
                ...
                <Row gutter={16}>
                    ...
                    <Form.Item
                      ...
                      <Select
                        ...
                        onChange={(value) => {
                          ...
                          const currentDateOfBirth = form.getFieldValue([
                            "secondPerson",
                            "dateOfBirth",
                          ]);
                  ...
                  {/*tự hiện khi nhập ngày sinh trên 18 tuổi */}
                  {isSecondPersonAdult && (
                    <Col xs={24} md={12}>
                      <Form.Item
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 sticky top-6">
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              {selectedCollectionMethod &&
                selectedCollectionMethod.price > 0 &&
                !isExpressService && (
                  <div className="flex justify-between text-sm">
                    <span>Collection Fee:</span>
                    <span>
                      {selectedCollectionMethod.price.toLocaleString()} đ
                    </span>
                  </div>
                )}
              ...
              {selectedMedicationMethod === "staff-collection" && (
                <div className="flex justify-between text-sm font-medium text-green-600 bg-green-50 p-2 rounded-md">
                  ...
                  <span>{isExpressService ? "Free" : "500,000 đ"}</span>
                </div>
              )}
              ...
              {isExpressService && (
                <div className="text-sm text-green-600 bg-green-50 p-2 rounded-md mt-2 italic text-center">
                  💡 When using Express Service, all sample collection and
                  shipping methods are Free
                </div>
              )}
              ...
              <div className="space-y-3">
                <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedMedicationMethod === "postal-delivery" && (
                      <span className="text-xs text-red-500 block mt-1 break-words">
                        (Not available with Postal Delivery)
                      </span>
                    )}
                  </div>
                </div>
                <div className="flex items-start max-w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedMedicationMethod === "postal-delivery" && (
                      <span className="text-xs text-green-500 block mt-1 break-words">
